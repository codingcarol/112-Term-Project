...
def make_times_list(schedule_folder,event_id):
...
def get_priority_list(schedule_folder,event_id):
    #returns the priority list of a specified event 
    times_set = make_times_list(schedule_folder,event_id)
...
def flex_schedule_helper(event_ids, priority_lists, schedule, weekday_dict):
    ...
    if check_schedule_done(event_ids):
        ...
        '''for ids in priority_lists:
            ...
            valid = is_valid_move(ids, event_ids[ids], move, schedule, weekday_dict)
            #print(weekday_dict)
            break'''
        return None
...
def is_valid_move(ids, event_info, move, schedule, weekday_dict):
...
def get_possible_times(priority_lists, strict_schedule, event_ids):
    #from the priority lists, give the times that are possible that week
    for i in priority_lists:
        print(i, priority_lists[i])
    print(strict_schedule)
    print(event_ids)
...
def generate_flexible_schedule(schedule_folder, strict_schedule, week):
    ...
    get_possible_times(priority_lists, strict_schedule, event_ids)
    flexible_schedule = flex_schedule_helper(event_ids, priority_lists, strict_schedule, weekday_dict)
    #return priority_lists
print(get_all_priority_lists("sample_schedule1", get_current_date()))